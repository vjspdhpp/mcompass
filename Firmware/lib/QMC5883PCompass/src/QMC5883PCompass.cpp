...
Modified for QMC5883P by Gemini.
...
void QMC5883PCompass::init() {
  Wire.begin();
  // Based on the original QMC5883L init(), we try to match the settings:
  // Continuous mode, 200Hz ODR, 8G Range, and highest OSR.
...
int QMC5883PCompass::getAzimuth() {
  float heading = atan2(getX(), getY()) * 180.0 / PI; // 已修正
