...
int QMC5883PCompass::getAzimuth()
{
    /* ------------------------------------------------------------------
       1️⃣  取出经校准后的二维分量
           - 翻到背面 ⇒ Y 轴方向反了，要取 “-Y”
           - North 轴 =  getX()   （板子箭头 Y 方向）
           - East  轴 = -getY()   （板子箭头 X 方向，取反）
    ------------------------------------------------------------------ */
    const float north = getX();      // +北 / –南
    const float east  = -getY();     // +东 / –西  ← 反号‼️

    /* ------------------------------------------------------------------
       2️⃣  计算从 “北” 顺时针转过的角度（0-360°）
           atan2( east, north )   就是航向角：北=0°, 东=90°, 南=180°…
    ------------------------------------------------------------------ */
    float heading = atan2(east, north) * 180.0f / PI;
    if (heading < 0.0f) heading += 360.0f;     // 折叠到 0-360

    /* ------------------------------------------------------------------
       3️⃣  把整圈再平移 180° → 让 “南” 成 0°
           结果刻度：南=0°, 西=90°, 北=180°, 东=270°
    ------------------------------------------------------------------ */
    heading += 180.0f;
    if (heading >= 360.0f) heading -= 360.0f;

    /* ------------------------------------------------------------------
       4️⃣  若需要加入地磁偏角，就放在 *最前面* 或 *最后面* 均可：
           heading += declination;  // 东偏为正，西偏为负
           同样再做一次折叠即可。
    ------------------------------------------------------------------ */

    return static_cast<int>(heading);          // 返回 0-359 的整数角度
}
